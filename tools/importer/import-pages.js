...
export default {
  transform: ({
    ...
    if (main.querySelectorAll('iframe') && (main.querySelectorAll('iframe').length > 0)) main.querySelectorAll('iframe').forEach((el) => el.remove());
    if (main.querySelector('div#onetrust-consent-sdk')) main.querySelector('div#onetrust-consent-sdk').remove();
    ...
    //    .replace('-pdf-coredownload-inline', '');
    // const pdfUrl = new URL(WebImporter.FileUtils.sanitizePath(pdfUrlPath), 'https://main--merative2--hlxsites.hlx.page')
    //        .toString();
    // const pdfCells = [
    //   ['PDF Viewer'],
    ...
    // const ctaLinkText =
    //    main.querySelector('.cmp-experiencefragment--consultation a .cmp-button__text');
    // let ctaTextInner = '';
    ...
    const newDocPath = WebImporter.FileUtils.sanitizePath(new URL(url).pathname);
    ...
    //   const newPath = WebImporter.FileUtils
    //      .sanitizePath(u.pathname.replace('-pdf-coredownload-inline', ''));
    ...
    main.querySelectorAll('a').forEach((a) => {
      ...
      if (href && href.endsWith('.pdf')) {
        ...
        const newPathUpdated = u.pathname.replace('.pdf.coredownload.inline', '').replace('-pdf-coredownload-inline', '');
        const newPath = WebImporter.FileUtils.sanitizePath(newPathUpdated);
        // no "element", the "from" property is provided instead -
        // importer will download the "from" resource as "path"
        results.push({
          path: newPath,
